    ...
    public int getFieldID(Context context, String field) throws InvalidMetadataFieldException, SQLException {
        ...
        if (!manager.hasField(field))
        {
            ...
            if (pieces.length > 1)
            {
                ...
                if (iterator.hasNext())
                {
                    manager.add(field, iterator.next().getIntColumn("mid"));
                }
                else
