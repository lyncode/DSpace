...
%>
		<div class="row">
			<div class="col-lg-12 form-group">
				...
				<table class="table">
					<thead>
						<th><fmt:message key="jsp.submit.edit-metadata.lastname" /></th>
						<th><fmt:message key="jsp.submit.edit-metadata.firstname" /></th>
						<th></th>
					</thead>
					<tbody>
<% 
...
%>
							<td>
								<input class="form-control" type="text" <% if (readonly) { %>disabled<% } %> name="<%=last.toString() %>" value="<%=dpn.getLastName().replaceAll("\"", "&quot;") %>"  />
							</td>
							<td>
								<input class="form-control" type="text" <% if (readonly) { %>disabled<% } %> name="<%=first.toString() %>" value="<%=dpn.getFirstNames().replaceAll("\"", "&quot;") %>"  />
							</td>
							<td align="right">
									<%
										request.setAttribute("fieldInput", fieldName);
										request.setAttribute("idx", i);
										request.setAttribute("fieldCount", fieldCount);
										request.setAttribute("confidenceValue", conf);
										request.setAttribute("authorityValue", auth);
										request.setAttribute("collection", collectionID);
										request.setAttribute("isName", true);
									%>
									<jsp:include page="authority/authority.jsp" />
