...
%>
							<tr>
								<td>
									<input type="text" name="<%=fieldName %>_series<%=(repeatable && i!= fieldCount) ? (i+1):"" %>" <% if (readonly) { %>disabled<% } %> value="<%=sn.getSeries().replaceAll("\"", "&quot;") %>" class="form-control" />
								</td>
								<td>
									<input type="text" name="<%=fieldName %>__number"<%=(repeatable && i!= fieldCount) ? (i+1):"" %>" <% if (readonly) { %>disabled<% } %> value="<%=sn.getNumber().replaceAll("\"", "&quot;") %>" class="form-control" />
								</td>
<% 
								if (repeatable && !readonly) {
%>
								<td>
									<button class="btn form-control btn-danger" type="submit" name="submit_<%=fieldName%>_remove_<%=i%>"><i class="icon-trash"></i></button>
								</td>
<%	
								}
%>
